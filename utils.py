...
def collate_test(data, user_neg):
    ...
    for da in data:
        ...
        user.append(da[4])
        length.append(da[5])
    return (torch.Tensor(user_alis).long(), dgl.batch_hetero(graph), torch.Tensor(last_item).long(), torch.Tensor(label).long(), torch.Tensor(length).long(), torch.Tensor(neg_generate(user, user_neg)).long())
...
def eval_metric(all_top, all_label, all_length, random_rank=True):
    ...
    return (np.mean(recall5), np.mean(recall10), np.mean(recall20), np.mean(ndgg5), np.mean(ndgg10), np.mean(ndgg20), pd.DataFrame(data_l, columns=['r5', 'r10', 'r20', 'n5', 'n10', 'n20', 'number']))
...
def format_arg_str(args, exclude_lst, max_len=20):
    ...
    res_str += ' ' + key_title + ' ' * (key_max_len - len(key_title)) + ' | ' + value_title + ' ' * (value_max_len - len(value_title)) + ' ' + linesep + '=' * horizon_len + linesep
    for key in sorted(keys):
        value = arg_dict[key]
        if value is not None:
            key, value = (str(key), str(value).replace('\t', '\\t'))
            value = value[:max_len - 3] + '...' if len(value) > max_len else value
            res_str += ' ' + key + ' ' * (key_max_len - len(key)) + ' | ' + value + ' ' * (value_max_len - len(value)) + linesep
    res_str += '=' * horizon_len
    return res_str
